...
@IonicPage()
@Component({
  selector: 'page-addLicense',
  templateUrl: 'addLicense.html',
  styleUrls: [ 'addLicense.scss', '../../../theme/variables.scss' ]
})
export class AddLicensePage {
  // @ViewChild(Content) content: Content;

  fleetDashboard: string = "info";
  ...
  showToast() {
    ...
    this.dbService.fetchDataV2("select * from fleetverification where id = " + this.fleetId.id)
      ...
      .subscribe(res => {
        ...
        // this.getfleetPens();
        this.loading.dismiss();
        this.navCtrl.push(AddLicensePage, {fleetDashboard: 'condition', fleet: this.fleet});
        this.showToast();
        }, error => {
        alert("FAILED, to fetch from php : "+ error);
        this.myreturnvalue = "Error : " + error;
